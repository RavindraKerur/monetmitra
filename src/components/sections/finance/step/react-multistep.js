...
export default function MultiStep(props) {
    ...
    props.steps.map((s, i) => (
      ...
      >
        <Link to="#"> <span className="number" />
          <span className="step-name">{s.text}</span>
        </Link>
      </li>
    ))
    ...
    show && (
      <div className="step-footer step-tab-pager">
        <button
          ...
          onClick={previous} className="btn-first btn-submit-fill"
        >
          Previous
        </button> 
        ...
        <button
          ...
          onClick={next} className="btn-first btn-submit-fill"
        >
          Next
        </button>
      </div>
  ...
  return (
    <div onKeyDown={handleKeyDown} id="add-user-tab" className="step-app">
      <Ol className="step-steps custom mb-xl-40">{renderSteps()}</Ol>
